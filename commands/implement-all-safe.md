---
description: Portable runtime-agnostic SAFE version— repeatedly run /implement-next-safe on a plan file until every task is complete. Works in any harness (Claude Code, Cursor, claude -p, etc.) without hook dependencies.
---
...
#### 3. **Recovery check — verify the task landed.**
   ...
   - If one of them missing, then **you MUST go to the step 3 ("Implement next task") again and do all fo the steps again. This is non-negotiable.**
