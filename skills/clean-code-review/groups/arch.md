...
**How to check**: For each file added whole in the diff (no counterpart in HEAD), first check whether its path contains a **layer-denoting** segment — a folder whose name maps to exactly one layer: `Presentation`, `Views`, `Screens`, `Pages`, `Components`, `Routes`, `ViewModels`, `ui`, `server`, `api`, `Controllers` (presentation); `Adapters`, `Infrastructure`, `Repositories`, `db`, `migrations` (infrastructure); or `application`, `use_cases`, `usecases`, `domain`, `entities` (core). Match segment names case-insensitively. A folder whose name maps to no single layer (e.g. `Models/`, `lib/`, `shared/`, `common/`) is not layer-denoting — skip it. If it does, classify the file's content with the layer classifier above. Flag only when the content layer is unambiguous AND differs from the layer its folder denotes.
