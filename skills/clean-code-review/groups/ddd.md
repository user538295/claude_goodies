...
NOTE for agent: only flag methods on classes that appear to be aggregate roots or entities (named after domain concepts). Dismiss repository query methods — their purpose is to return collections. All languages now require an **empty parameter list**: an aggregate's collection accessor returns its own collection and takes no arguments, whereas a repository query takes filters (`get_all_vectors(self, collection)`, `getItemsFor(String status)`). Those are no longer matched. A zero-arg `getAll()` on an entity *is* still matched and is a genuine violation — the argument count, not the `All` in the name, is what distinguishes a query from an accessor. Dismiss if the return type is already an immutable wrapper (`IReadOnlyList`, `IReadOnlyCollection`, `IEnumerable`, `Iterable`, `Sequence`, `ImmutableList`, `List` (Java) when returned via `Collections.unmodifiableList`, etc.). Dismiss if the collection field is `private final` / `private val` and the method returns a copy.
