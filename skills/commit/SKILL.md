...
name: commit
description: >-
  Write standardized Git commit messages in a direct, why-first style — Conventional Commits subject + a body of one or more `## H2` sections, each opening with a context paragraph then bullets. Use when the user asks to "write a commit message", "draft a commit message", "what should the commit message be", "make the commit message", "commit message for these changes", or invokes /commit or /commit message. Two modes: bare `/commit` generates the message AND runs `git commit` (commit mode); `/commit message` generates message text only and never mutates the repo (draft mode). Safe to invoke as one step inside a larger task: it does its job, reports, and returns control without ever ending the agent's turn. Always applies this skill's standardized format — never inspects prior commits to "match local convention". Do NOT use for PR descriptions, tag annotations, or release notes — those have different voice constraints and live outside this skill.
---
...
- The literal argument `commit` — `/commit commit`. This is the **deterministic trigger for agent callers**: pass `commit` as the argument and commit mode is guaranteed.
...
**The rule:** bare `/commit` or an explicit commit *verb* directed at the repo → commit mode. `/commit message` or a message-only request → draft mode. When genuinely ambiguous, default to **commit mode** (the explicit action the user asked for) and say so.
...
**What gets committed (commit mode)** is decided from context, not from staging state alone: the changes produced during this conversation are the intended commit, whether they are staged, unstaged, or not yet tracked. Always reconcile that understanding against `git status` / the diff before committing — never commit from memory alone. **An explicit path or `@file` argument passed to the invocation is itself the clear set** — commit that scope directly and skip the ambiguity check below, even if the working tree holds other unrelated changes elsewhere; the user already named the scope, so asking again is a wasted round-trip. When the intended set is otherwise **clear**, stage exactly those files and commit. When it is **ambiguous** — the working tree holds changes you did not make, several unrelated change-sets are present, or the session context isn't available to identify the set — **invoke the `/options` skill** to present the candidate change-sets and let the user choose; commit only what they select.
...
8. **Commit (commit mode only).** Commit the exact message you just emitted:
   - **Decide what to commit (context first, not staging state).** The intended commit is the set of changes produced during this conversation — staged, unstaged, or untracked alike. Cross-check that against `git status --short` and the diff; **never commit from memory alone.** An explicit path/`@file` argument in the invocation is a clear set on its own — use it directly, without the ambiguous-set check, regardless of what else is unstaged in the working tree.
