...
`start-watch` checks the skill path via `ls ~/.claude/skills/llm-wiki-product/watcher.py` first; falls back to `<repo>/.claude/skills/llm-wiki-product/watcher.py`.
...
5. Run: `nohup python3 ~/.claude/skills/llm-wiki-product/watcher.py start <project-root> > /dev/null &` (stderr is NOT redirected so startup errors are visible in the terminal before daemonizing).
