...
# ---------------------------------------------------------------------------
# test_commit_msg_param_skips_prompt
# COMMIT_MSG set; commit_and_push must use it without reading stdin.
# ---------------------------------------------------------------------------
@test "test_commit_msg_param_skips_prompt" {
  _stage_file "noninteractive.txt"

  COMMIT_MSG="automated commit"

  # No stdin provided — would hang if read is called
  run bash -c "
    source '$SCRIPT'
    CLAUDE_DIR='$CLAUDE_DIR'
    CONF_FILE='$CONF_FILE'
    COMMIT_MSG='automated commit'
    git() {
      local arg; for arg in \"\$@\"; do [[ \"\$arg\" == push ]] && return 0; done
      command git \"\$@\"
    }
    export -f git
    commit_and_push
  ...
  local last_msg
  last_msg=$(git -C "$CLAUDE_DIR" log --oneline -1)
  [[ "$last_msg" == *"automated commit"* ]]
}

# ---------------------------------------------------------------------------
# test_commit_msg_param_auto_pushes
# COMMIT_MSG set; commit_and_push must push without prompting.
# ---------------------------------------------------------------------------
@test "test_commit_msg_param_auto_pushes" {
  _stage_file "autopush.txt"

  run bash -c "
    source '$SCRIPT'
    CLAUDE_DIR='$CLAUDE_DIR'
    CONF_FILE='$CONF_FILE'
    COMMIT_MSG='auto push commit'
    push_called=false
    git() {
      local arg
      for arg in \"\$@\"; do
    ...
    export -f git
    commit_and_push
    echo \"push_called=\$push_called\"
  "

  [ "$status" -eq 0 ]
  [[ "$output" == *"push_called=true"* ]]
}

# ---------------------------------------------------------------------------
# test_message_flag_via_main
# --message flag passed to main() commits non-interactively and auto-pushes.
# ---------------------------------------------------------------------------
@test "test_message_flag_via_main" {
  _stage_file "flagged.txt"

  run env CLAUDE_DIR="$CLAUDE_DIR" CONF_FILE="$CONF_FILE" \
    _TEST_BASH_MAJOR=4 bash "$SCRIPT" --message "flag commit msg"

  [ "$status" -eq 0 ]
  local last_msg
  last_msg=$(git -C "$CLAUDE_DIR" log --oneline -1)
  [[ "$last_msg" == *"flag commit msg"* ]]
}

# ---------------------------------------------------------------------------
# test_short_m_flag_via_main
# -m flag is an alias for --message.
# ---------------------------------------------------------------------------
@test "test_short_m_flag_via_main" {
  _stage_file "shortflag.txt"

  run env CLAUDE_DIR="$CLAUDE_DIR" CONF_FILE="$CONF_FILE" \
    _TEST_BASH_MAJOR=4 bash "$SCRIPT" -m "short flag msg"

  [ "$status" -eq 0 ]
  local last_msg
  last_msg=$(git -C "$CLAUDE_DIR" log --oneline -1)
  [[ "$last_msg" == *"short flag msg"* ]]
}

# ---------------------------------------------------------------------------
# test_message_flag_requires_value
# --message without a value exits 1.
# ---------------------------------------------------------------------------
@test "test_message_flag_requires_value" {
  run bash "$SCRIPT" --message

  [ "$status" -eq 1 ]
  [[ "$output" == *"requires"* ]] || [[ "$output" == *"Usage"* ]]
}
